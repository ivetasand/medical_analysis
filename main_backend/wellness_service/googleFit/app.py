...
from main_backend.wellness_service.googleFit.app_decorator import login_required, login_login
import json
import time
...
google = oauth.register(
    ...
    client_kwargs={'scope': 'email profile https://www.googleapis.com/auth/fitness.activity.write https://www.googleapis.com/auth/fitness.activity.read'}, #добавить scope из API
...
def authorize():
    ...
    r = requests.post("http://127.0.0.1:7000/login_data_post/", data={result_list})
...
def authorization(token):
    ...
    body = {
        ...
        "startTimeMillis": int(time.mktime(two_months_ago.timetuple()) / 86400) * 86400 * 1000,
        "endTimeMillis": int(time.mktime(time.localtime()) / 86400) * 86400 * 1000
...
def parse(res):
    ...
    for i in range(len(seq)):
        ...
        if "startTimeMillis" in seq[i]:
            startTimeMillis = int(seq[i].strip().split(": ")[1].strip('"').replace('"', '').split(",")[0])
            if startTimeMillis != "":
                startTimeMillis = datetime.fromtimestamp(startTimeMillis / 1000).strftime('%Y-%m-%d')
